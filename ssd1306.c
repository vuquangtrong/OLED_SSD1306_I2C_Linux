...
// Do not use SMBUs because the screen buffer is more than I2C_SMBUS_BLOCK_MAX
...
// 
int i2c_open(const char *device, int devId) {
    int fd ;
...
static inline int i2c_rdwr(int fd) {
...
void SSD1306_CMD_Send() {
  if (SSD1306.Device > 0) {
    if (write(SSD1306.Device, SSD1306_Buffer.CmdBuffer, SSD1306_Buffer.CmdBufferIndex) != SSD1306_Buffer.CmdBufferIndex) {
...
void SSD1306_DATA_Send() {
  if (SSD1306.Device > 0) {
    if (write(SSD1306.Device, SSD1306_Buffer.DataBuffer, 1+SSD1306_BUFFER_DATA_MAX) != 1+SSD1306_BUFFER_DATA_MAX) {
